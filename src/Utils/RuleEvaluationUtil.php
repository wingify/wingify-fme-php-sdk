    ...
    public static function evaluateRule(ServiceContainer $serviceContainer, $feature, $campaign, $context, &$evaluatedFeatureMap, &$megGroupWinnerCampaigns, $storageService, &$decision, bool $isDebuggerUsed = false)
    {
        ...
        if ($preSegmentationResult && DataTypeUtil::isObject($whitelistedObject) && count((array) $whitelistedObject) > 0) {
            ...
            if(!$isDebuggerUsed) {
                ...
                $payload = $networkUtil->getTrackUserPayloadData($serviceContainer->getSettings(), EventEnum::VARIATION_SHOWN, $campaign->getId(), $whitelistedObject['variation']->getId(), $context);
            }
        }
