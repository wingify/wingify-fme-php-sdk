...
class WingifyBuilder implements IWingifyBuilder
{
    protected $options;
        ...
        while (true) {
            ...
            try {
                ...
                if ($stringifiedLatestSettings !== $lastSettings) {
                    ...
                    SettingsUtil::setSettingsAndAddCampaignsToRules($clonedSettings, $this->clientInstance, $this->logManager);
                }
            } catch (\Exception $error) {
                $this->logManager->error('Error while fetching settings with polling');
            }
        }
    }
    ...
    {
        return new WingifyClient($this->settings, $this->options, $this->serviceContainer);
    }
    ...
    public function build($settings)
    {
        ...
        $this->clientInstance = $this->instantiateClient();
