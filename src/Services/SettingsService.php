...
use wingify\Packages\NetworkLayer\Models\RequestModel;
use Exception;
use wingify\Models\Schemas\SettingsSchema;
...
class SettingsService implements ISettingsService {
    ...
    /** @var string Host for settings / edge API (v2-settings, gateway helper GETs, aliasing) */
    public $hostname = Constants::LEGACY_HOST;
    /** @var string Host for event collection POSTs (/events/t, batch events) */
    public $eventsHostname = Constants::LEGACY_HOST;
    public $hostProfile = HostProfileEnum::WINGIFY;
    /** @var string SDK name sent in network payloads (sn, sd, sdk, etc.) */
    public $sdkName = Constants::SDK_NAME;
    /** @var string SDK version sent in network payloads */
    public $sdkVersion;
    public $port;
    ...
    public function __construct($options, $logManager, $loggerService) {
        ...
        $profile = isset($options['hostProfile']) ? $options['hostProfile'] : HostProfileEnum::WINGIFY;
        $this->hostProfile = $profile;
        $this->applyPlatformHosts($profile);

        $this->sdkName = ComposerUtil::getSdkName();
        $this->sdkVersion = ComposerUtil::getSdkVersion();
        ...
        if (isset($options['proxy']['url']) && !empty($options['proxy']['url']) && !$this->isGatewayServiceProvided) {
            ...
            try {
                ...
                if ($parsedUrl !== false && isset($parsedUrl['host'])) {
                    ...
                    $this->syncEventsHostToSettingsHost();
                } else {
        ...
        $this->settingsSchemaValidator = new SettingsSchema();
    ...
    /**
     * @param string $profile HostProfileEnum::VWO or HostProfileEnum::WINGIFY
     */
    private function applyPlatformHosts($profile)
    {
        if ($profile === HostProfileEnum::VWO) {
            $this->hostname = Constants::LEGACY_HOST;
            $this->eventsHostname = Constants::LEGACY_HOST;
            return;
        }

        $this->hostname = Constants::EDGE_HOST;
        $this->eventsHostname = Constants::COLLECT_HOST;
    }

    private function syncEventsHostToSettingsHost()
    {
        $this->eventsHostname = $this->hostname;
    }

    public function usesCustomerHostOverride()
    {
        return $this->isGatewayServiceProvided || $this->isProxyUrlProvided;
    }

    public function getSettingsHostname()
    {
        return $this->hostname;
    }

    public function getEventsHostname()
    {
    ...
    private function initializeNetworkManager($options) {
        ...
        NetworkManager::instance($networkOptions);
    }
    ...
    private function fetchSettingsAndCacheInStorage() {
        try {
            ...
            if ($this->settingsSchemaValidator->isSettingsValid($settings)) {
                $this->isSettingsValidOnInit = true;
                $this->loggerService->info('SETTINGS_FETCH_SUCCESS');
                return $settings;
            } else {
    ...
    public function fetchSettings() {
        ...
        $options['sn'] = $this->sdkName;
        $options['sv'] = $this->sdkVersion;
